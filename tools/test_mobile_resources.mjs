...
function handle(value) {
  if (value.type === "extension_ui_request" && value.method === "setWidget" && value.widgetKey === "__android_loaded_resources") {
    if (!initialWidget) {
      ...
      assert.ok(section(initialWidget, "Skills").includes("demo-skill"));
      assert.ok(section(initialWidget, "Prompts").includes("/review"));
    ...
    if (!refreshedWidget && value.widgetLines.some(line => line === "[Context]")) {
      ...
      assert.ok(section(refreshedWidget, "Skills").includes("demo-skill"));
      assert.ok(section(refreshedWidget, "Prompts").includes("/review"));
